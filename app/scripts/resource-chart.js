...
// var myChart = echarts.init(document.getElementsByClassName('resource-chart'));
var resourceChart = echarts.init(document.getElementById('chart02'));
var resourceOption;
...
var resourceChartColors = [
    '#ccb24a', '#8ecc4e', '#67d779', '#65bcc9', '#666cdf', '#9e66cb', '#aa5bb6', '#bb5d6c', '#c14e34', '#d7a04f',
    '#e2d97b', '#b9d790', '#a5d7ad', '#9ccccd', '#898cbf', '#c0a0db', '#eabde6', '#bb908c', '#d2977f', '#f8f1b3',
    '#f1e423', '#87ff37', '#41d748', '#3aa8b8', '#2f3abb', '#8737b8', '#7c007a', '#ac242d', '#691e08', '#9d6f16'
];

// $.get(ROOT_PATH + '/data/asset/data/airport-schedule.json', function (rawData)
$.get('./data/resource-usage.json', function (rawData) {
    _rawData = rawData;
    resourceChart.setOption((resourceOption = makeOption2()));
    initDrag();
});
        ...
        series: [
            {
                id: 'flightData',
                type: 'custom',
                renderItem: renderGanttItem2,
                dimensions: _rawData.flight.dimensions,
                encode: {
                    x: [DIM_TIME_ARRIVAL, DIM_TIME_DEPARTURE],
                    y: DIM_CATEGORY_INDEX,
                    tooltip: [DIM_CATEGORY_INDEX, 3, DIM_TIME_ARRIVAL, DIM_TIME_DEPARTURE]
                },
...
function renderGanttItem2(params, api) {
        ...
        children: [
            {
                ...
                ignore: !rectNormal,
                shape: rectNormal,
                style: api.style({ fill: resourceChartColors[api.value(3)] })
            },
            // {
            //     type: 'rect',
            //     ignore: !rectVIP && !api.value(4),
            //     shape: rectVIP,
            //     style: api.style({ fill: '#3f75dd' })
            // },
            {
                type: 'rect',
                ignore: !rectText,
...
function initDrag() {
    ...
    resourceChart.on('mousedown', function (param) {
    ...
    resourceChart.getZr().on('mousemove', function (event) {
        if (!_draggingEl) {
            return;
        }
    ...
    resourceChart.getZr().on('mouseup', function () {
        // Drop
    ...
    resourceChart.getZr().on('globalout', dragRelease);
    function dragRelease() {
        _autoDataZoomAnimator.stop();
        if (_draggingEl) {
            resourceChart.getZr().remove(_draggingEl);
            _draggingEl = null;
        }
        if (_dropShadow) {
            resourceChart.getZr().remove(_dropShadow);
            _dropShadow = null;
        }
        _dropRecord = _draggingRecord = null;
    }
    function addOrUpdateBar(el, itemData, style, z) {
        var pointArrival = resourceChart.convertToPixel('grid', [
            itemData.timeArrival,
            itemData.categoryIndex
        ]);
        var pointDeparture = resourceChart.convertToPixel('grid', [
        ...
        var barHeight =
            Math.abs(
                resourceChart.convertToPixel('grid', [0, 0])[1] -
                resourceChart.convertToPixel('grid', [0, 1])[1]
            ) * HEIGHT_RATIO;
        if (!el) {
    ...
    function prepareDrop() {
        ...
        var cursorData = resourceChart.convertFromPixel('grid', [xPixel, yPixel]);
        if (cursorData) {
    ...
    function dispatchDataZoom(params) {
        var resourceOption = resourceChart.getOption();
        var optionInsideX = resourceOption.dataZoom[DATA_ZOOM_X_INSIDE_INDEX];
        var optionInsideY = resourceOption.dataZoom[DATA_ZOOM_Y_INSIDE_INDEX];
        var batch = [];
        prepareBatch(
            batch,
            'insideX',

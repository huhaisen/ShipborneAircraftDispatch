...
// var myChart = echarts.init(document.querySelector(".aircraft-chart"));
var aircraftChart = echarts.init(document.getElementById('chart01'));
var aircraftOption;
...
var aircraftChartColors = new Map([
    ["航电1", '#ccb24a'],
    ["航电2", '#e2d97b'],
    ["航电3", '#f1e423'],
    ["航电4", '#8ecc4e'],
...
$.get('./data/airport-schedule.json', function (rawData) {
    _rawData = rawData;
    aircraftChart.setOption((aircraftOption = makeOption()));
    initDrag();
});
function makeOption() {
    return {
        ...
        series: [
            {
                ...
                encode: {
                    ...
                    tooltip: [DIM_CATEGORY_INDEX, 3, 4, DIM_TIME_ARRIVAL, DIM_TIME_DEPARTURE]
                },
...
function renderGanttItem(params, api) {
    ...
    var barColor;
    var resourceGroup = api.value(4);
    barColor = aircraftChartColors.get(resourceGroup);
...
function initDrag() {
    ...
    aircraftChart.on('mousedown', function (param) {
    ...
    aircraftChart.getZr().on('mousemove', function (event) {
        if (!_draggingEl) {
            return;
        }
    ...
    aircraftChart.getZr().on('mouseup', function () {
        // Drop
    ...
    aircraftChart.getZr().on('globalout', dragRelease);
    function dragRelease() {
        _autoDataZoomAnimator.stop();
        if (_draggingEl) {
            aircraftChart.getZr().remove(_draggingEl);
            _draggingEl = null;
        }
        if (_dropShadow) {
            aircraftChart.getZr().remove(_dropShadow);
            _dropShadow = null;
        }
        _dropRecord = _draggingRecord = null;
    }
    function addOrUpdateBar(el, itemData, style, z) {
        var pointArrival = aircraftChart.convertToPixel('grid', [
            itemData.timeArrival,
            itemData.categoryIndex
        ]);
        var pointDeparture = aircraftChart.convertToPixel('grid', [
        ...
        var barHeight =
            Math.abs(
                aircraftChart.convertToPixel('grid', [0, 0])[1] -
                aircraftChart.convertToPixel('grid', [0, 1])[1]
            ) * HEIGHT_RATIO;
        if (!el) {
    ...
    function prepareDrop() {
        ...
        var cursorData = aircraftChart.convertFromPixel('grid', [xPixel, yPixel]);
        if (cursorData) {
    ...
    function dispatchDataZoom(params) {
        var aircraftOption = aircraftChart.getOption();
        var optionInsideX = aircraftOption.dataZoom[DATA_ZOOM_X_INSIDE_INDEX];
        var optionInsideY = aircraftOption.dataZoom[DATA_ZOOM_Y_INSIDE_INDEX];
        var batch = [];
        prepareBatch(
            batch,
            'insideX',
...
aircraftOption && aircraftChart.setOption(aircraftOption);
// aircraftChart.setOption(aircraftOption);
